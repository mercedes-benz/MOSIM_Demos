...
public class PhotoboothScriptSceneGen: MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //if (start)
        //{
        //    Debug.Log(gameObjs[ObjectNum].name);
        //    areaMeshTriangle(gameObjs[ObjectNum]);
        //}
        ...
        if (Input.GetKeyUp(KeyCode.Space) || TakePic.readyForNextPart == true)
        {
    ...
    IEnumerator startPhotoSession()
    {
        ...
        if (currentObjIndex == 0)
        {
            ...
            //EditorBuildSettingsScene[] newSceneList = new EditorBuildSettingsScene[EditorBuildSettings.scenes.Length + 1];
            //for (int i = 0; i < EditorBuildSettings.scenes.Length; i++)
            //{
            //    newSceneList[i] = EditorBuildSettings.scenes[i];
            //}
            //newSceneList[EditorBuildSettings.scenes.Length] = new EditorBuildSettingsScene(newSceneStr,true);
        ...
        else if (currentObjIndex == numOfObjs)
        {
            ...
            //        (VariableScript.projectedArea[i]*ProjectedAreaWeight + 
            //        VariableScript.ratioVisibleSurfaceArea[i]*VisibleSurfaceAreaWeight +
            //        (VariableScript.centerOfMassY[i] + VariableScript.centerOfMassX[i])/2*CenterOfMassWeight);
            //}


            //// Looks through each picture for each object; for each set of pictures for the objects, determines the highest 'score' and the index of that score in the overall array of values. Pulls the correct file from the screenshots and saves to the "Best Views" folder
            //string newPath = Application.dataPath + "/Screenshots/BestViews/";
            ...
            //{
            //    //Debug.Log(gameObjs[i].name + ": " + VariableScript.numOfPicturesPerObj[i]);
            //    //Debug.Log("Total: " + total);
            //    float[] subsetOfQualityOfView = new float[VariableScript.numOfPicturesPerObj[i]];
            //    Array.Copy(VariableScript.QualityOfView, total, subsetOfQualityOfView, 0, VariableScript.numOfPicturesPerObj[i]);
            //    if (subsetOfQualityOfView.Length != 0)
            //    {
            //        float maxValue = subsetOfQualityOfView.Max();
            //        int maxIndex = subsetOfQualityOfView.ToList().IndexOf(maxValue) + total;
            //        saveBestViews(VariableScript.pictureFilePath[maxIndex], VariableScript.pictureFileName[maxIndex], newPath);
            //    }
            //    //Debug.Log(VariableScript.pictureFileName[maxIndex].ToString() + ": " + maxValue.ToString("F4"));
            //    yield return new WaitUntil(() => picCopied == true);
            //    picCopied = false;
            //    total += VariableScript.numOfPicturesPerObj[i];
            //}
            ...
            //Debug.Log("Projected Area Time: " + VariableScript.sw_projectedarea.Elapsed.TotalMilliseconds.ToString("F4") + " (" + (VariableScript.sw_projectedarea.Elapsed.TotalMilliseconds / totalTime).ToString("F2") + ")");
            //Debug.Log("CoM Time: " + VariableScript.sw_com.Elapsed.TotalMilliseconds.ToString("F4") + " (" + (VariableScript.sw_com.Elapsed.TotalMilliseconds / totalTime).ToString("F2") + ")");
            //Debug.Log("Get Pixels: " + VariableScript.sw_getpixels.Elapsed.TotalMilliseconds.ToString("F4") + " (" + (VariableScript.sw_getpixels.Elapsed.TotalMilliseconds / totalTime).ToString("F2") + ")");
            //Debug.Log("Bounds: " + VariableScript.sw_bounds.Elapsed.TotalMilliseconds.ToString("F4") + " (" + (VariableScript.sw_bounds.Elapsed.TotalMilliseconds / totalTime).ToString("F2") + ")");
            //Debug.Log("Mesh: " + VariableScript.sw_mesh.Elapsed.TotalMilliseconds.ToString("F4") + " (" + (VariableScript.sw_mesh.Elapsed.TotalMilliseconds / totalTime).ToString("F2") + ")");
            

            // For checking values of the metrics

            //for (int i = 0; i < VariableScript.projectedArea.Length; i++)
            //{
            //    Debug.Log(VariableScript.projectedArea[i]);
            //    Debug.Log(VariableScript.ratioVisibleSurfaceArea[i]);
            //}
